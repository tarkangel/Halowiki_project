...
The entire wiki is available in English and Latin American Spanish, toggled with a single button persisted to `localStorage`:

- **`src/contexts/LanguageContext.tsx`** — a React context exposing `lang` (`'en' | 'es'`) and `toggleLang()`. Language preference is stored under `halowiki-lang` in `localStorage` and restored on next visit.
- **Parallel data files** — every entity JSON has a Spanish counterpart under `src/data/es/`. `fetchCharacters(lang)`, `fetchWeapons(lang)`, etc. in `static.ts` branch on `lang` and return the appropriate array, so components need no extra logic.
- **Inline `t` objects** — each page defines a small `{ en, es }` translation object for UI strings (titles, subtitles, load-more labels). Sidebar navigation labels and faction tags are also bilingual.
- **Data parity enforcement** — EN and ES JSON files are kept in strict ID-level sync. Any entry present in one language but absent in the other is removed from both to prevent ghost entries in one locale.
...
| Data | Static JSON (pre-fetched by CI, bundled at build time — EN + ES) |
...
│   ├── characters.json            # English
│   ├── weapons.json
│   ├── vehicles.json
│   ├── races.json
│   ├── planets.json
│   ├── games.json
│   └── es/                        # Latin American Spanish counterparts (ID-level parity enforced)
│       ├── characters.json
│       ├── weapons.json
│       ├── vehicles.json
│       ├── races.json
